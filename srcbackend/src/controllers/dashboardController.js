...
const { pdvScope, withScope, estGlobal } = require('../utils/scope');

/** Inclusion PDV scopée, prête à l'emploi pour les requêtes Vente/Alerte/Position du dashboard. */
...
const dashboardController = {
  async getKPIs(req, res) {
    try {
      ...
      const pdvActifs = await PDV.count({ where: withScope({ statut: 'actif' }, scope) });
      ...
      const alertesActives = await Alerte.count({
  ...
  async getPDVStats(req, res) {
    try {
      ...
      const [tagues, actifs, inactifs, suspendus] = await Promise.all([
        PDV.count({ where }),
        PDV.count({ where: withScope({ ...where }, { statut: 'actif' }) }),
        PDV.count({ where: withScope({ ...where }, { statut: 'inactif' }) }),
        PDV.count({ where: withScope({ ...where }, { statut: 'suspendu' }) })
      ]);
  ...
  async getVentesParZone(req, res) {
    try {
      ...
      const scoped = Object.keys(scope).length > 0;
      const ventes = await Vente.findAll({
        include: [{ model: PDV, as: 'pdv', where: scoped ? scope : undefined, required: scoped, include: ['zone'] }]
      });
