...
const KPI_CARDS = [
  { key: 'pdv_actifs', label: 'PDV Actifs', icon: MapPin, iconBg: 'bg-primary-50', iconColor: 'text-primary-600' },
  { key: 'ventes_aujourdhui', label: "Ventes Aujourd'hui", icon: ShoppingCart, iconBg: 'bg-success-50', iconColor: 'text-success-600' },
  { key: 'alertes_actives', label: 'Alertes Actives', icon: AlertTriangle, iconBg: 'bg-danger-50', iconColor: 'text-danger-600' },
  { key: 'total_pdv', label: 'Total PDV', icon: Activity, iconBg: 'bg-warning-50', iconColor: 'text-warning-600' },
];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-ink-500 mt-0.5">Vue d'ensemble de votre activité terrain</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {KPI_CARDS.map(({ key, label, icon: Icon, iconBg, iconColor }) => (
          <div key={key} className="card !p-5">
            ...
            <p className="text-3xl font-bold text-ink-900 leading-none">{values[key]}</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-ink-500 mt-1">PDV tagués</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-success-50 text-success-600 shrink-0">
              <Activity className="w-5 h-5" />
            </span>
            <div>
              <p className="text-2xl font-bold text-ink-900 leading-none">{pdvStats?.actifs ?? 0}</p>
              <p className="text-xs text-ink-500 mt-1">PDV actifs</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-ink-100 text-ink-500 shrink-0">
              <UserX className="w-5 h-5" />
            </span>
            <div>
              <p className="text-2xl font-bold text-ink-900 leading-none">{pdvStats?.inactifs ?? 0}</p>
              <p className="text-xs text-ink-500 mt-1">PDV inactifs</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="flex items-center justify-center w-10 h-10 rounded-xl bg-danger-50 text-danger-600 shrink-0">
              <AlertTriangle className="w-5 h-5" />
            </span>
            <div>
              <p className="text-2xl font-bold text-ink-900 leading-none">{pdvStats?.suspendus ?? 0}</p>
              <p className="text-xs text-ink-500 mt-1">PDV suspendus</p>
            </div>
          </div>
        </div>
      </div>
            ...
            {produitsChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={produitsChartData} layout="vertical" margin={{ left: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#EDECF7" />
          ...
          {analysesLoading ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analysesChartData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#EDECF7" />
